...
Creer le fichier _postcss.config.js_
...
L'objectif est de faire en sorte que les données inscritent dans les inputs se retrouvent dans le cadre de droite.
...
Pour qu'une fonction soit accessible avec des evenements de type onclick() alors elle doit etre dans le scope global de webpack.
